...
fn draw_select_screen(
    ...
    // Title
    let title_block = create_block("MONSTER SIREN RECORDS", COLOR_PRIMARY);
    ...
    let list_block = create_block("BROWSE ALBUMS", COLOR_MUTED);
    ...
    let status_block = create_block("STATUS", COLOR_MUTED);
    ...
    // Help bar
    let help_block = create_block("CONTROLS", COLOR_MUTED);
...
fn draw_download_screen(
    ...
    let title_block = create_block(
        "MSR TRANSFER CONSOLE",
        if done { COLOR_SUCCESS } else { COLOR_WARNING },
    );
    ...
    let title = Paragraph::new(title_text)
        .style(
            Style::default()
                .fg(if done { COLOR_SUCCESS } else { COLOR_WARNING })
                .add_modifier(Modifier::BOLD),
        )
        .block(title_block);
    f.render_widget(title, chunks[0]);
    ...
    if task_items.is_empty() {
        ...
        let placeholder = Paragraph::new(text).block(create_block(&task_title, COLOR_MUTED));
    ...
    let help_block = create_block("CONTROLS", COLOR_MUTED);
    let help_text = if let Some(error) = progress.errors.last() {
        format!("TAB ALBUMS  Q QUIT  /  LAST ERROR: {}", error)
    } else if done {
        "TAB ALBUMS  Q QUIT  /  TRANSFER COMPLETE".to_string()
    } else {
        format!(
            "TAB ALBUMS  Q QUIT  /  {} ALBUM{} LEFT  /  {} TRACK{} LEFT",
                ...
                if event::poll(std::time::Duration::from_millis(50))? {
                    match event::read()? {
                        Event::Key(key) if key.kind == KeyEventKind::Press => match key.code {
                            ...
                            KeyCode::Char('a') => {
                                let all_selected = selected_albums.iter().all(|&s| s);
                                for s in &mut selected_albums {
                                    *s = !all_selected;
                                }
                            }
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(names) = cli.album {
        downloader::download_albums_by_name(&api, &config, &names).await?;
    } else {
        downloader::download_all(&api, &config).await?;
    }
